...
#define PONCHA110_ANALOG_GAIN_MAX 1
...
#define PONCHA110_BANK_SEL_REG 0xE000
#define PONCHA110_RW_CONTEXT_REG 0xE004
#define PONCHA110_CMD_REQ_1_REG 0x000A
#define PONCHA110_CMD_HALT_BLOCK_REG 0x000C
 ...
 */
#define PONCHA110_SUPPORTED_MODE_SIZE_PUBLIC 1
static const struct poncha110_mode supported_modes[] = {
	{
		...
		.reg_list_pre_soft_reset = {
			.num_of_regs = ARRAY_SIZE(full_10b_1lane_reg_pre_soft_reset),
			.regs = full_10b_1lane_reg_pre_soft_reset,
		},
...
struct poncha110
{
	...
	// custom v4l2 control
	struct v4l2_ctrl *poncha110_reg_w;
	struct v4l2_ctrl *poncha110_reg_r;
	u16 poncha110_reg_w_cached_addr;
	u8 poncha110_reg_w_cached_flag;
...
static int poncha110_otp_read(struct poncha110 *poncha110, u8 addr, u32 *val)
{
	...
	int poll_cnt_max = 10;
	int ret;
	poncha110_write(poncha110, PONCHA110_BANK_SEL_REG, 0);
...
static int poncha110_v4l2_reg_w(struct poncha110 *poncha110, u32 value)
{
	...
	else if (reg_flag & AMS_CAMERA_CID_PONCHA110_REG_FLAG_FOR_READ)
	{
		...
		poncha110->poncha110_reg_w_cached_addr = reg_addr;
		poncha110->poncha110_reg_w_cached_flag = reg_flag;
	}
	else
	{
		...
		if ((reg_flag & AMS_CAMERA_CID_PONCHA110_REG_FLAG_I2C_SEL) == AMS_CAMERA_CID_PONCHA110_REG_FLAG_I2C_MIRA)
		{
			...
			if (reg_flag & AMS_CAMERA_CID_PONCHA110_REG_FLAG_USE_BANK)
			{
				...
				ret = poncha110_write(poncha110, PONCHA110_BANK_SEL_REG, bank);
				if (ret)
				{
				...
				ret = poncha110_write(poncha110, PONCHA110_RW_CONTEXT_REG, context);
				if (ret)
				{
...
static int poncha110_v4l2_reg_r(struct poncha110 *poncha110, u32 *value)
{
	...
	u16 reg_addr = poncha110->poncha110_reg_w_cached_addr;
	u8 reg_flag = poncha110->poncha110_reg_w_cached_flag;
	u8 reg_val = 0;
...
static int poncha110_write_exposure_reg(struct poncha110 *poncha110, u32 exposure)
{
	...
	// ret = poncha110_write(poncha110, PONCHA110_RW_CONTEXT_REG, 1);
	// ret = poncha110_write_be32(poncha110, PONCHA110_EXP_TIME_L_REG, exposure);
	if (ret)
	{
...
static int poncha110_write_stop_streaming_regs(struct poncha110 *poncha110)
{
	...
	// // Set conetxt bank 0 or 1
	// ret = poncha110_write(poncha110, PONCHA110_BANK_SEL_REG, 0);
	// if (ret)
	// {
	// 	dev_err(&client->dev, "Error setting BANK_SEL_REG.");
	// 	return ret;
	...
	// ret = poncha110_write(poncha110, PONCHA110_CMD_HALT_BLOCK_REG,
	// 					1);
	// if (ret)
	// {
	...
	/*
	u32 frame_time;
		frame_time = PONCHA110_DEFAULT_FRAME_LENGTH *
			PONCHA110_DEFAULT_LINE_LENGTH / PONCHA110_DEFAULT_PIXEL_CLOCK;
								  ...
								  struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->pad == IMAGE_PAD)
	{
				 ...
				 poncha110->fmt.code != fmt->format.code)
		{
			...
			// rc = __v4l2_ctrl_s_ctrl(poncha110->vblank, PONCHA110_DEFAULT_VBLANK_60);
			// if (rc)
			// {
...
static int poncha110_init_controls(struct poncha110 *poncha110)
{
	...
	int ret;
	struct v4l2_ctrl_config *poncha110_reg_w;
	struct v4l2_ctrl_config *poncha110_reg_r;
	 ...
	 * Custom op
	 */
	poncha110_reg_w = &custom_ctrl_config_list[0];
	printk(KERN_INFO "[PONCHA110]: %s AMS_CAMERA_CID_MIRA_REG_W %X.\n", __func__, AMS_CAMERA_CID_MIRA_REG_W);
	poncha110->poncha110_reg_w = v4l2_ctrl_new_custom(ctrl_hdlr, poncha110_reg_w, NULL);

	poncha110_reg_r = &custom_ctrl_config_list[1];
	printk(KERN_INFO "[PONCHA110]: %s AMS_CAMERA_CID_MIRA_REG_R %X.\n", __func__, AMS_CAMERA_CID_MIRA_REG_R);
	poncha110->poncha110_reg_r = v4l2_ctrl_new_custom(ctrl_hdlr, poncha110_reg_r, NULL);
	if (poncha110->poncha110_reg_r)
		poncha110->poncha110_reg_r->flags |= (V4L2_CTRL_FLAG_VOLATILE | V4L2_CTRL_FLAG_READ_ONLY);
...
static int poncha110_probe(struct i2c_client *client)
{
	...
	poncha110->row_length = PONCHA110_ROW_LENGTH;
